...
class Game():
    ...
    def __init__(self, test, small_blind, max_hands, starting_chips=20000):
        '''
    ...
    def showdown(self, players):
        score_to_kicker = {1:4,2:3,3:1,4:2,5:0,6:4,7:0,8:1,9:0,10:0}
        player_score = {}
        if len(players) == 0:
...
class ManualPlayer(Player):
    ...
    def getActionType(self,preflop, to_act_after, out, community_cards, potsize, cbet):
        action = input('Action: ')
        #amount = input('Amount: ')
...
class RandomPlayer(Player):
    ...
    def getActionType(self, p_round, action_left, out, community_cards, pot, cbet):
...
class NeuralPlayer(Player):
    def __init__(self, name):
        super().__init__(name)
        self.preflop_network = PokerNetwork(7, 5, 3)
        self.postflop_network = PokerNetwork(6, 5, 3)
        self.bot = True
    ...
    def getActionType(self, preflop, to_act_after, out, community_cards, potsize, cbet):
        if preflop:
            inputs = [int(CheckAce(self.hand.first)), int(CheckAce(self.hand.second)), int(JackOrBetter(self.hand.first)), int(JackOrBetter(self.hand.second)), ConvToRange(to_act_after), ConvToRange(out), ConvToRange(potsize)]
            output = self.preflop_network.feedForward(inputs)
        else:
            cards = self.get_hand() + community_cards
            inputs = [int(MadeHand(cards)), int(AtLeastTwoPair(cards)), int(AtLeastFlush(cards)), ConvToRange(to_act_after), ConvToRange(out), ConvToRange(potsize)]
