...
def eaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen, stats=None,
    ...
    for gen in range(1, ngen + 1):
        ...
        if len(hof) != 0:
            for i in range(20):
                offspring.append(tools.selRandom(hof, 1)[0])
        ...
        # apply deterministic crowding to the selected population: if an offpsring pareto dominated its parent but was not selected
        # then it will be now

        population = applyDeterministicCrowding(population, offspring)
        ...
        pickle.dump(population, open("saves/One-Population Coevolution with NGSA and Mu+Lambda and deterministic crowding and HOF/population_gen%i.p" % gen, 'wb'))
