...
def eaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen, stats=None,
    ...
    while len(results) < total_players*2: # play as many games in the population as there are players, so that everyone has a chance to play everyone else
    ...
    for gen in range(1, ngen + 1):
        ...
        for i,p in enumerate(population):
            pickle.dump(p, open("Gathering Results/saves/Six-Population Pareto Coevolution NGSA/Attempt 2/population%i_gen%i.p" % (i, gen), 'wb'))
